  ...
  async function handleFiles(files: FileList | null): Promise<void> {
    ...
    for (const [index, file] of selectedFiles.entries()) {
      try {
        ...
        const booking = extractBookingFromPdfText({
          fileName: file.name,
          text,
          options: {
            kind: form.kind,
        ...
        newRows.push(toUiBooking(booking, rows.length + index));
      } catch (caughtError) {
    ...
    <div style={pageStyle}>
      <header style={{ marginBottom: 20 }}>
        ...
        <p style={{ margin: 0, maxWidth: 760, color: "#4b5563", fontSize: 16, lineHeight: 1.55 }}>
          Lade Rechnungs-PDFs hoch, prüfe die erkannten Buchungssätze und lade danach eine EXTF-CSV für den Import herunter.
        </p>
      </header>
      ...
      <section style={{ ...cardStyle, marginBottom: 16 }}>
        ...
        <p style={{ margin: "12px 0 0", color: "#6b7280", fontSize: 14, lineHeight: 1.5 }}>
          Hinweis: Das funktioniert direkt im Browser für Text-PDFs. Reine Scan-PDFs brauchen später zusätzlich OCR.
        </p>
