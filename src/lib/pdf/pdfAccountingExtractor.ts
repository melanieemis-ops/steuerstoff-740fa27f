...
export function extractBankStatementBookings(input: {
  ...
  const transactions = blocks
    .map((block) => parseBankTransactionBlock(block))
  ...
  if (uniqueTransactions.length === 0) {
    throw new Error(`${input.fileName}: Kontoauszug erkannt, aber keine einzelnen Umsätze gefunden.`);
  }
...
function isPotentialBankTransactionStart(line: string): boolean {
  ...
  return collectBankMoneyAmounts(line).length > 0 ||
    /lastschrift|gutschrift|kartenzahlung|überweisung|ueberweisung|dauerauftrag|einzug|abbuchung|zahlung/i.test(line);
}
...
function chooseBankTransactionAmount(amounts: MoneyMatch[]): MoneyMatch {
  const explicitSigned = amounts.filter((amount) => /(^[+\-−])|([\-−]\s*(?:€|EUR)?$)|\b(?:Soll|Haben|S|H)\b/i.test(amount.raw));
  return explicitSigned.at(-1) ?? amounts[amounts.length - 1];
}

function collectBankMoneyAmounts(text: string): MoneyMatch[] {
  const result: MoneyMatch[] = [];
  const regex = /([+\-−]?\s*\d{1,3}(?:[.\s]\d{3})*,\d{2}|[+\-−]?\s*\d+,\d{2})(?:\s*(?:€|EUR))?(?:\s+(Soll|Haben|S|H)\b)?/gi;

  for (const match of text.matchAll(regex)) {
    const raw = match[1];
    const signToken = match[2];
    const parsed = parseSignedGermanBankAmount(raw, signToken, text);
    if (parsed === undefined || !Number.isFinite(parsed)) continue;
    result.push({ value: parsed, raw: match[0].trim(), index: match.index ?? 0 });
  }
...
function buildBankPurpose(rawBlock: string, rawAmount: string): string {
  ...
  text = text.replace(/\b(?:buchungstag|buchung|wertstellung|valuta|umsatz|betrag|soll|haben|eur|€)\b/gi, " ");
...
function collectDates(text: string): Date[] {
  ...
  const fallbackYear = inferYearFromText(text) ?? new Date().getFullYear();
  const shortDateRegex = /\b(\d{1,2})[.\-/](\d{1,2})\.?(?!\d)/g;
  for (const match of text.matchAll(shortDateRegex)) {
    const date = buildDate(Number(match[1]), Number(match[2]), fallbackYear);
    if (date) result.push(date);
  }
