...
export const Route = createFileRoute("/lerngebiete")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Wähle deinen Lernbereich in der steuerstoff Akademie.",
      },
    ],
  }),
});
...
const SUBJECTS: Subject[] = [
  {
    ...
    title: "Umsatzsteuer",
    shortTitle: "USt",
    description: "Steuerbarkeit, Leistungsort, Steuersätze, Rechnungen, Vorsteuer und Reverse Charge.",
    icon: ReceiptText,
  ...
  },
  {
    ...
    shortTitle: "AO",
    description: "Verwaltungsakte, Fristen, Einspruch, Verjährung und Korrekturvorschriften.",
    icon: Scale,
  ...
  },
  {
    ...
    shortTitle: "ESt",
    description: "Steuerpflicht, Einkunftsarten, Gewinnermittlung, Werbungskosten und Betriebsausgaben.",
    icon: Calculator,
  ...
  },
  {
    ...
    title: "Lohnsteuer",
    shortTitle: "LSt",
    description: "Arbeitslohn, Sachbezüge, Dienstwagen, ELStAM, Reisekosten, Pauschalierung und Lohnabrechnung.",
    icon: FileText,
  ...
  },
  {
    ...
    title: "Gewerbesteuer",
    shortTitle: "GewSt",
    description: "Gewerbeertrag, Hinzurechnungen, Kürzungen, Freibetrag und Messbetrag.",
    icon: Building2,
  ...
  },
  {
    ...
    shortTitle: "ErbSt",
    description: "Steuerklassen, Freibeträge, Bewertung, Erwerbe und Verschonungsregeln.",
    icon: Landmark,
  ...
  },
  {
    ...
    shortTitle: "NPO",
    description: "Sphären, Mittelverwendung, Rücklagen, Zweckbetriebe und Spendenrecht.",
    icon: HeartHandshake,
            ...
            <div className="mt-6 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <p className="mt-3 max-w-2xl text-sm leading-relaxed text-muted-foreground sm:text-base">
                  Wähle ein Fachgebiet und starte eine zufällige Lernrunde. Bereiche ohne Fragen bleiben sichtbar und werden automatisch freigeschaltet, sobald Inhalte vorhanden sind.
                </p>
              </div>
              ...
              <div className="inline-flex w-fit items-center gap-2 rounded-2xl border border-border bg-background px-4 py-3 shadow-sm">
                ...
                <div>
                  <p className="text-[11px] font-medium text-muted-foreground">Aktuell verfügbar</p>
                  <p className="text-sm font-semibold text-foreground">{learningQuestions.length} Fragen</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {SUBJECTS.map((subject) => {
              const questionCount = learningQuestions.filter(
                (question) => question.category === subject.category,
              ).length;
              return (
                <SubjectCard
...
function SubjectCard({ subject, questionCount }: { subject: Subject; questionCount: number }) {
    ...
    return (
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <span className={`inline-flex h-12 w-12 items-center justify-center rounded-2xl ${subject.iconClass}`}>
            <Icon className="h-6 w-6" />
          </span>
          <span className={`rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide ${subject.accentClass}`}>
            Verfügbar
          </span>
        </div>
        <div className="mt-5">
          <p className="text-xs font-semibold uppercase tracking-[0.15em] text-muted-foreground">{subject.shortTitle}</p>
          <h2 className="mt-1 text-xl font-semibold text-foreground">{subject.title}</h2>
          <p className="mt-3 text-sm leading-relaxed text-muted-foreground">{subject.description}</p>
        </div>
        <div className="mt-auto pt-6">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-2xl font-semibold tabular-nums text-foreground">{questionCount}</p>
